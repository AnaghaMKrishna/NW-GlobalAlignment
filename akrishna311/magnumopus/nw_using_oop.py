...
class NeedlemanWunsch:
    ...
    def fill_matrix(self, score_mat: list[list[int]], dir_mat:list[list[list[int]]], seq_a: str, seq_b: str, match: int, mismatch: int, gap: int) -> tuple[list[list[int]], list[list[list[int]]]]:
        """
    ...
    def traverse(self, cell:list[int], path:int, row:int, col:int, aln_a:str, aln_b:str, dir_mat:list[list[list[int]]], seq_a:str, seq_b:str) -> tuple[str,str]:
        """
        ...
        while path >= 0:
            ...
            if cell[path] == 0:
                return self.traverse(dir_mat[row-1][col-1], path, row-1, col-1, aln_a + seq_a[row-1], aln_b + seq_b[col-1], dir_mat, seq_a, seq_b)
            #if right, add base from seq_b to aln_b_temp and gap on aln_a_temp
            elif cell[path] == 1:
                return self.traverse(dir_mat[row][col-1], path, row, col-1, aln_a + "-", aln_b + seq_b[col-1], dir_mat, seq_a, seq_b)
            #if down, add base from seq_a to aln_a_temp and gap on aln_b_temp
            elif cell[path] == 2:
                return self.traverse(dir_mat[row-1][col], path, row-1, col, aln_a + seq_a[row-1], aln_b + "-", dir_mat, seq_a, seq_b)
        path -= 1


    def traverse_all_paths(self, cell:list[int], path:int, row:int, col:int, aln_a:str, aln_b:str, dir_mat:list[list[list[int]]], seq_a:str, seq_b:str) -> tuple[str, str]:
        """
        ...
        if cell[path] == 0:
            ...
            #print(f"{aln_a_temp}\n{aln_b_temp}")
            while path > 0:
                yield from self.traverse_all_paths(cell, path - 1, new_row, new_col, aln_a_temp, aln_b_temp, dir_mat, seq_a, seq_b)
                path -= 1
            ...
            #print(f"{aln_a_temp}\n{aln_b_temp}")
            while path > 0:
                yield from self.traverse_all_paths(cell, path - 1, row, new_col, aln_a_temp, aln_b_temp, dir_mat, seq_a, seq_b)
                path -= 1
            ...
            #print(f"{aln_a_temp}\n{aln_b_temp}")
            while path > 0:
                yield from self.traverse_all_paths(cell, path - 1, new_row, col, aln_a_temp, aln_b_temp, dir_mat, seq_a, seq_b)
                path -= 1


    def find_optimal_alignment(self, score_mat: list[list[int]], dir_mat: list[list[list[int]]], seq_a: str, seq_b: str, all:bool) -> tuple[tuple[str, str], int] | tuple[list[tuple[str, str], int]]:
        """
        ...
        if all:
            ...
            all_alns = [i for i in self.traverse_all_paths(cell, path, row, col, aln_a, aln_b, dir_mat, seq_a, seq_b)]
            return all_alns, score
        else:
            #return only one alignment if --all flag is not set
...
arg_seq1 = parser.add_argument(
    "-s1", "--sequence1",
    help="first DNA sequence to align",
    dest="seq1",
    type=str,
    required=True
)

arg_seq2 = parser.add_argument(
    "-s2", "--sequence2",
    help="second DNA sequence to align",
    dest="seq2",
    type=str,
    required=True
)
...
arg_one_or_all = parser.add_argument(
    "--all",
    help="indicate --all to return all optimal alignments, returns one alignment by default",
    dest="one_or_all",
    required=False,
    action="store_true"
)
